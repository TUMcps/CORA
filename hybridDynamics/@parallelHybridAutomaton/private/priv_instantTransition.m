function [mergedTrans,tracker] = priv_instantTransition(pHA,locID,allLabels,tracker)
% priv_instantTransition - if the currently active locations allow for an
...
%
% Syntax:
%    [mergedTrans,tracker] = priv_instantTransition(pHA,list,locID,allLabels,tracker)
%
% Inputs:
...
for k=1:length(pHA.components)
    ...
    if idxIdReset(k)
        ...
        stateDim = pHA.components(k).location(locID(k)).contDynamics.nrOfDims;
