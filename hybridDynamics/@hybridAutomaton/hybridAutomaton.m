...
%
% Inputs:
%    loc - cell-array storing location objects
%
% Outputs:
...
methods
    ...
    function HA = hybridAutomaton(varargin)
        ...
        elseif ~iscell(varargin{1}) ...
                || ~all(cellfun(@(x) isa(x,'location'),varargin{1},'UniformOutput',true))
            throw(CORAerror('CORA:wrongInputInConstructor,',...
                'Input argument has to be a cell array of location objects.'));
        end
        
        HA.location = varargin{1};
        
        % check if locations are well-defined
