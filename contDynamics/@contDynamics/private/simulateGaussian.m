function res = simulateGaussian(obj, params, options)
...
for i = 1:options.points
    ...
    for iChange = 1:totalInputChanges-1
        ...
        % set initial state
        if iChange == 1
            options.x0 = randPoint(options.R0,'gaussian',options.p_conf);
            t{i}(1) = startTime;
            x{i}(1,:) = options.x0;
        else
        ...
        if isfield(options,'U') && ~isempty(options.U)
            % set input
            uRand = randPoint(options.U,'gaussian',options.p_conf);
        ...
        if isfield(options,'W') && ~isempty(options.W)
            % add to constant input
            options.w = randPoint(options.W,'gaussian',options.p_conf);
        else
            options.w = [];
        end
