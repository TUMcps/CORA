...
%         options.intermediateOrder = 10;
        ...
        %separate evaluation
        H_mid = cell(sys.nrOfDims,1);
        H_rad = cell(sys.nrOfDims,1);
        for i=1:sys.nrOfDims
        ...
        %interval evaluation
        error_rad = zeros(sys.nrOfDims,1);
        for i=1:sys.nrOfDims
...
elseif options.tensorOrder == 3
    ...
    if isempty(cell2mat(ind))
        ...
        errorLagr = zonotope(zeros(sys.nrOfDims,1));
    else
        % skip tensors with all-zero entries using ind from tensor creation
        errorLagr = interval(zeros(sys.nrOfDims,1),zeros(sys.nrOfDims,1));
        for i=1:length(ind)
