function [timeInt,timePoint,res,savedata] = priv_reach_adaptive(linsys,params,options)
...
set.Pu = zeros(sys.nrOfDims,1);
set.Putotal = zeros(sys.nrOfDims,1);
set.PU = [];
set.PUtotal = zeros(sys.nrOfDims,1);
...
set.G_PUtotal_infty = zeros(sys.nrOfDims,1);
set.G_PUtotal_zero = double.empty(sys.nrOfDims,0);
set.Gabs_PUtotal_zero = double.empty(sys.nrOfDims,0); % only omega_rad
set.Ghat_PUtotal_zero = double.empty(sys.nrOfDims,0); % only omega_max
...
if ~isU
    set.eAtkPU = zeros(linsys.nrOfDims,1);
...
% - consists of eps_linComb + 2*err(F H*) + 2*err(G u) + eps_PU_tauk
    ...
    try
        F = priv_correctionMatrixState(linsys,timeStep,Inf);
    catch ME
...
% initialize errors
G_PU_infty = [];
A_sum_error = zeros(linsys.nrOfDims);
