function [V,H,krylovOrder] = priv_subspace_Krylov_individual_Saad(A,nu_A,v,initKrylovOrder,options)
% priv_subspace_Krylov_individual_Saad - computes the Krylov subspace for a single
...
%
% Syntax:
%    [V,H,krylovOrder] = priv_subspace_Krylov_individual_Saad(A,nu_A,v,initKrylovOrder,options)
%
% Inputs:
%    A - system matrix
...
%                02-June-2025 (MP, Change name to _Saad, see also _Jawecki method)
% Last revision: ---
...
krylovOrder = initKrylovOrder - 2*options.krylovStep;
if krylovOrder <= 0
    krylovOrder = 1;
end
    ...
    if any(any(isinf(supremum(exp_Htau_I.int))))
        ...
        if ~happyBreakdown
            for iStep=1:timeSteps
                ...
                % exponential matrix
                exp_tH = expm(H*t);
                if ~any(any(isnan(exp_tH))) && ~any(any(isinf(exp_tH))) 
