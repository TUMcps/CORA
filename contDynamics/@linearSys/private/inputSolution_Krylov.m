function linsys = inputSolution_Krylov(linsys,options)
...
eqivState = [zeros(linsys.nrOfStates,1); 1];
...
if ~all(withinTol(U_c,0))
    ...
    Usum = zeros(linsys.nrOfStates,1);
    errorTaylor_c = zeros(linsys.nrOfStates,1);
    KrylovOrderConst = 1;
end
...
for iGen = 1:nrOfGens
    ...
    if ~all(withinTol(U_g,0))
        ...
        A_int_g = [linsys.A, U_g; zeros(1,linsys.nrOfStates), 0];
        ...
        [V_Ug,H_Ug,KrylovOrder] = ...
            subspace_Krylov_individual(A_int_g,nu_A_int_g,eqivState,KrylovOrder,options);
        ...
        for i=1:options.taylorTerms
            ...
            % compute sums
            Ugen(:,end+1) = U_unprojected(1:linsys.nrOfStates); 
        end
        ...
        errorTaylor_sum = errorTaylor_sum + errorTaylor_g(1:linsys.nrOfStates);
    end
    iGen
end
...
if ~all(withinTol(vTrans,0)) % input trajectory not yet implemented
    ...
    if options.originContained
        ...
        inputCorr = zeros(linsys.nrOfStates,1);
    else
        % obtain constant input solution
    ...
    inputSolVtrans = zeros(linsys.nrOfStates,1);
    inputCorr = zeros(linsys.nrOfStates,1);
    errorTaylor_vTrans = zeros(linsys.nrOfStates,1);
end
...
if options.krylovError > 2*eps
    ...
    errorRed = interval(-ones(linsys.nrOfStates,1),ones(linsys.nrOfStates,1)) * ...
...
function [Usum, errorTaylor, inputCorr, KrylovOrder] = ...
    ...
    % equivalent initial state
    eqivState = [zeros(sys.nrOfStates,1); 1];
    
    % init Usum
    Usum = zeros(sys.nrOfStates,1);

    % create integrator system
    A_int = [sys.A, input; zeros(1,sys.nrOfStates), 0];
    ...
    % Arnoldi
    [V_U,H_U,KrylovOrder] = ...
        subspace_Krylov_individual(A_int,nu_A_int,eqivState,KrylovOrder,options);
    ...
    for i=1:options.taylorTerms
        ...
        % compute sums
        Usum = Usum + U_unprojected(1:sys.nrOfStates); 
    end
    
    % error due to finite Taylor series
    errorTaylor_unproj = V_U*E(:,1);
    errorTaylor = errorTaylor_unproj(1:sys.nrOfStates);
    ...
    if inputTieFlag
        ...
        inputCorr_unprojected = interval(...
            ...
            inputTie_error_mid + inputTie_error_rad); 
        inputCorr = zonotope(inputCorr_unprojected(1:sys.nrOfStates)); 
    end
end
