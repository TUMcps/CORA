function [V_c,H_c,V_g,H_g] = priv_subspace_Krylov(linsys,Z,options)
% priv_subspace_Krylov - computes the Krylov subspaces for the center and
%    generators of a zonotpe
%
% Syntax:
%    [V_c,H_c,V_g,H_g] = priv_subspace_Krylov(linsys,Z,options)
%
% Inputs:
...
if c_norm == 0
    ...
    [V_c,H_c,KrylovOrder] = priv_subspace_Krylov_individual(A,nu_A,c,KrylovOrder,options);
...
if nrOfGens>0
    for iGen = 1:nrOfGens
        ...
        if g_norm == 0
            ...
            %Arnoldi
            [V_g{iGen},H_g{iGen},KrylovOrder] = priv_subspace_Krylov_individual(A,nu_A,G(:,iGen),KrylovOrder,options);
            % change results back \dot{x} = Ax (Krylov papers use \dot{x} = -Ax);
            % V remains unchanged
            %H_g{iGen} = -H_g{iGen};
        end
