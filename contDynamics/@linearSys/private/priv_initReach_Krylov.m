function [Rfirst,options] = priv_initReach_Krylov(linsys,Rinit,options)
% priv_initReach_Krylov - computes the reachable continuous set for the first
%    time step using Krylov subspace methods
%
% Syntax:
%    [Rfirst,options] = priv_initReach_Krylov(linsys,Rinit,options)
%
% Inputs:
...
[V_c,H_c,V_g,H_g] = priv_subspace_Krylov(linsys,Rinit,options);
...
%     error = norm(Rinit)*error_normalized;
    ...
    R_initSol = zonotope(c_new,G_new,err*eye(linsys.nrOfDims));
else
    R_Krylov = 0;
...
[V_c,H_c,V_g,H_g] = priv_subspace_Krylov(linsys,RV_0,options);
