function [V_c,H_c,V_g,H_g] = priv_subspace_Krylov_Saad(linsys,Z,options)
% priv_subspace_Krylov_Saad - computes the Krylov subspaces for the center and
%    generators of a zonotpe
%
% Syntax:
%    [V_c,H_c,V_g,H_g] = priv_subspace_Krylov_Saad(linsys,Z,options)
%
% Inputs:
...
% Last update:   02-June-2025 (MP, Change name to priv_subspace_Krylov_Saad to differentiate from Jawecki bound)
% Last revision: ---
...
if c_norm == 0
    ...
    [V_c,H_c,KrylovOrder] = priv_subspace_Krylov_individual_Saad(A,nu_A,c,KrylovOrder,options);
...
if nrOfGens>0
    for iGen = 1:nrOfGens
        ...
        if g_norm == 0
            ...
            %Arnoldi
            [V_g{iGen},H_g{iGen},KrylovOrder] = priv_subspace_Krylov_individual_Saad(A,nu_A,G(:,iGen),KrylovOrder,options);
            % change results back \dot{x} = Ax (Krylov papers use \dot{x} = -Ax);
            % V remains unchanged
            %H_g{iGen} = -H_g{iGen};
        end
