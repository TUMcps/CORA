function [res,R] = verify(sys,params,spec)
% verify - verification for linear systems
%
% Syntax:  
...
%------------- BEGIN CODE --------------
    ...
    [params.safeSet,params.unsafeSet] = getSetsFromSpec(spec);
    % idea: allow to overwrite initial error (skipping initError call)

    % input argument pre-processing
    options = validateOptions(sys,mfilename,params,struct());
    
    % estimate the required max. allowed error with simulations
    emax = initError(sys,options,options.safeSet,options.unsafeSet);
    ...
    while true
        ...
        for i = 1:length(options.safeSet)
            for j = 1:length(timeInt.set)
                ...
                % verified before
                if ( isempty(options.safeSet{i}.time) || ...
                    intersectionCheckInterval(options.safeSet{i}.time,time(j,:)) ) ...
                        && checkSet(options.savedata.safeSet_unsat{i},time(j,:))
                
                    % outer-approximation
                    d = containmentCheckZono(timeInt.set{j},options.safeSet{i}.set);
                    ...
                    if d < 0
                        ...
                        options.savedata.safeSet_unsat{i} = ...
                            removeFromUnsat(options.savedata.safeSet_unsat{i},time(j,:));
                    ...
                    if d > timeInt.error(j)
                        ...
                        [R_i,ind_Ri,ind_now] = compInnerApprox(...
                            R_i,ind_Ri,timeInt.set,timeInt.error,j);
                        d = containmentCheckConZono(R_i{ind_now},options.safeSet{i}.set);
                        if d > 0
        ...
        for i = 1:length(options.unsafeSet)
            for j = 1:length(timeInt.set)
                ...
                if ( isempty(options.unsafeSet{i}.time) || ...
                        intersectionCheckInterval(options.unsafeSet{i}.time,time(j,:)) ) ...
                        && checkSet(options.savedata.unsafeSet_unsat{i},time(j,:))
                
                    % fast intersecting check for speed-up
                    if intersectionCheckFast(timeInt.set{j},options.unsafeSet{i})

                        % outer-approximation
                        d = distanceIntersectionZono(timeInt.set{j},options.unsafeSet{i}.set);
                        ...
                        [R_i,ind_Ri,ind_now] = compInnerApprox(...
                            R_i,ind_Ri,timeInt.set,timeInt.error,j);
                        d = intersectionCheck(R_i{ind_now},options.unsafeSet{i}.set);
                        ...
                        options.savedata.unsafeSet_unsat{i} = ...
                            removeFromUnsat(options.savedata.unsafeSet_unsat{i},time(j,:));
...
function [G,F] = getSetsFromSpec(spec)
% extract safe sets G and unsafe sets F from the specifications
    ...
    for i = 1:length(spec)
        if strcmp(spec(i).type,'safeSet')
            G{end+1}.set = normalizedRep(mptPolytope(spec(i).set));
            G{end}.time = spec(i).time;
        elseif strcmp(spec(i).type,'unsafeSet')
            tmp = normalizedRep(mptPolytope(spec(i).set));
            if size(tmp.P.A,1) > 1
                F{end+1}.set = tmp;
                F{end}.time = spec(i).time;
                if size(F{end}.set.P.A,1) > size(F{end}.set.P.A,2)
                ...
                G{end+1}.set = mptPolytope(-tmp.P.A,-tmp.P.b);
                G{end}.time = spec(i).time;
            end
        else
...
% initializations
function err = initError(sys,options,G,F)
% estimate the initial value for the maximum allowed error from simulations
    ...
    if ~isempty(g)
        ...
        if ~isInterval(Z)
            points = getInitPoints(options.R0,points);
        end
    else
        points = c;
    end
    ...
    for i = 1:size(points,2)
            ...
            for j = 1:size(y,1)
               if contains(F{k}.time,t(j))
                   ...
                   if contains(F{k}.set,y(j,:)')
                       temp = min(abs(F{k}.set.P.A*y(j,:)'-F{k}.set.P.b));
                       err = min(err,temp*scale);
                   elseif F{k}.isBounded 
                       if distanceIntervalPoint(F{k}.int,y(j,:)') < err
                           temp = distancePolyPoint(F{k}.set,y(j,:)');
                           err = min(err,temp*scale);
                       end
                   else
                       if size(F{k}.set.P.A,1) == 1
                           temp = F{k}.set.P.A*y(j,:)'-F{k}.set.P.b;
                       else
                           temp = distancePolyPoint(F{k}.set,y(j,:)');
                       end
                       err = min(err,temp*scale);
        ...
        for k = 1:length(G)
            if size(G{k}.set.P.A,1) == 1 && isempty(G{k}.time)
                scale = 10 - 9/t(end)*t;
                tmp = min(abs(G{k}.set.P.A*y'-G{k}.set.P.b),[],1);
                err_ = min(tmp .* scale');
                err= min(err,err_);
            else
                for j = 1:size(y,1)
                   if contains(G{k}.time,t(j))
                       if contains(G{k}.set,y(j,:)')
                           temp = min(abs(G{k}.set.P.A*y(j,:)'-G{k}.set.P.b));
                       elseif size(G{k}.set.P.A,1) == 1
                           temp = G{k}.set.P.A*y(j,:)'-G{k}.set.P.b;
                       else
                           temp = distancePolyPoint(G{k}.set,y(j,:)');
                       end
                       scale = 10 - 9/t(end)*t;
                       err = min(err,temp*scale); 
                   end
...
function points = getInitPoints(R0,points)
...
% inner-approximation
function [R_i,ind_Ri,ind_now] = compInnerApprox(R_i,ind_Ri,R_o,Rout_error,ind_Ro)
...
if ~isempty(ind_now)
    ...
    q = dim(R_o{ind_Ro});
    B = mptPolytope(sqrt(q)*Rout_error(ind_Ro)* [eye(q),-eye(q)]');
...
% R_i = cell(size(R_o));
% for i = 1:length(R_o)
%    B = mptPolytope(sqrt(n)*actError.timeInt(i)* [eye(n),-eye(n)]');
...
% containment checks
function dist = containmentCheckZono(Z,P)
...
% intersection checks
function dist = intersectionCheck(cZ,P)
...
function res = intersectionCheckFast(Z,F)
% fast hierarchical check if a zonotope intersects a polytope
    ...
    if F.isBounded
       if isIntersecting(interval(Z),F.int) && ...
            	intersectionCheck(Z,F.set) == 0 
            res = true;
       end
    elseif intersectionCheck(Z,F.set) == 0
        res = true;
    end
end

function res = intersectionCheckInterval(I1,I2)
...
function dist = distanceIntersectionZono(Z,P)
...
function d = distanceIntervalPoint(I,p)
...
function d = distancePolyPoint(P,p)
...
function doCheck = checkSet(set_unsat,t)
...
function set_unsat = removeFromUnsat(set_unsat,t)
