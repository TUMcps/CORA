function [VerrorDyn,VerrorStat,err,options] = priv_abstractionError_adaptive(...
...
if strcmp(options.alg,'lin') && options.tensorOrder == 2
    ...
    err = zeros(nlnsys.nrOfDims,1);
    if ~options.isHessianConst
        for i = 1:nlnsys.nrOfDims
            H_ = abs(H{i});
...
elseif strcmp(options.alg,'lin') && options.tensorOrder == 3
    ...
    if ~isempty(cell2mat(ind))
        errorLagr = interval(zeros(nlnsys.nrOfDims,1),zeros(nlnsys.nrOfDims,1));
        for i=1:length(ind)
