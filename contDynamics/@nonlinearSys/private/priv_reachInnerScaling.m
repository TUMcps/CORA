function [Rin,Rout] = priv_reachInnerScaling(nlnsys,params,options)
% priv_reachInnerScaling - compute an inner-approximation of the reachable set
...
% ------------------------------ BEGIN CODE -------------------------------
    ...
    for i = options.N:options.N:length(Rout_.timePoint.set)
        ...
        if input && ~representsa_(R,'emptySet',1e-12)
            set{end+1} = project(R,1:nlnsys.nrOfDims);
        else
            set{end+1} = R;
        end
