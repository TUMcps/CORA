...
% priv_identifySparse - Identifies a nonlinear system from trajectory data
%   using the SINDy approach [1]
%
% Syntax:
%    sys = priv_identifySparse(data,phi)
%
% Inputs:
...
% ------------------------------ BEGIN CODE -------------------------------
    ...
    for i = 1:size(points.dx,1)
        a = lasso(points.phi',points.dx(i,:)');
        A(i,:) = a(:,1)';
    end
...
% refine the sparse model using Dynamic Mode Decomposition (DMD)
    ...
    for i = 1:size(A,1)
        ...
        Anew{i} = aux_dynamicModeDecomposition(phi_,points.dx(i,:));
