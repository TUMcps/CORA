function example_linearParam_reach_ARCH20_platoon_PLAD01_BND42()
% example_linearParam_reach_ARCH20_platoon_PLAD01_BND42 -  example of linear
% reachability analysis from the ARCH20 friendly competition (platoon example);
% the linear dynamics can switch arbitrarily
%
% Syntax:  
%    example_linearParam_reach_ARCH20_platoon_PLAD01_BND42
%
% Inputs:
%    no
%
...
disp(['spefication verified: ',num2str(~violation42)]);
...
for i = 1:length(simRes)
    for j = 1:length(simRes{i}.t)
        plot(simRes{i}.t{j},simRes{i}.x{j}(:,1),'k');
    end
end
