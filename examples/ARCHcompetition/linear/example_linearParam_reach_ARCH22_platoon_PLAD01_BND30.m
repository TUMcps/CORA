function example_linearParam_reach_ARCH22_platoon_PLAD01_BND30()
% example_linearParam_reach_ARCH22_platoon_PLAD01_BND30 - example of linear
%    reachability analysis from the ARCH22 friendly competition
%    (platoon example); the linear dynamics switch deterministically
%
% Syntax:  
%    example_linearParam_reach_ARCH22_platoon_PLAD01_BND30
%
% Inputs:
%    no
%
...
for i = 1:length(simRes)
    for j = 1:length(simRes{i}.t)
        plot(simRes{i}.t{j},simRes{i}.x{j}(:,1),'Color',colorblind('y'));
    end
end
