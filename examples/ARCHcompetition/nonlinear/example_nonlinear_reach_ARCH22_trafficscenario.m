function text = example_nonlinear_reach_ARCH22_trafficscenario
% example_nonlinear_reach_ARCH22_trafficscenario - example of nonlinear
%    reachability analysis.
%
% Syntax:  
%    example_nonlinear_reach_ARCH22_trafficscenario()
%
% Inputs:
%    ---
%
...
text = ['CORA,TRAF22, ,',num2str(res),',',num2str(tComp)];

end

% Auxiliary Functions -----------------------------------------------------

function R = compReachSet(K,x0,u,t)
% compute the reachable set of the controlled car
    ...
    % system dynamics
    sys = nonlinParamSys(@controlledCar,10,7,12);
...
function dx = controlledCar(x,w,p)
% differential equation for the closed-loop system
    ...
    dx1 = car(x,w(1:2),u);
    dx2 = car(x_ref,zeros(2,1),u_ref);

    dx = [dx1; dx2];
end

function dx = car(x,w,u)
...
function [x0,u,K,t] = readSolutionFile(path)
...
function O = compOccupancySet(R,tay)
...
function tay = taylorOccupancySet()
...
function exportOccupancySet(fileSol,O)
...
function res = checkInputConstraints(R,u,K)
