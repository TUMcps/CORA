function [res, contagious] = priv_incrementalMultiBranch(R,analyzer,i,tFinal,verbose)
% priv_incrementalMultiBranch - incremental STL verification using a reachable set with multiple branches
%
% Syntax:
%   [res, contagious] = priv_incrementalMultiBranch(R,phi,i,tFinal,verbose)
%
% Inputs:
%    R - reachable set
...
for c = 1:length(cs)
    ...
    if tStart <= tStop
        [branch_res,branch_cont] = priv_incrementalMultiBranch(R,new_d,cs(c),tFinal,verbose);
