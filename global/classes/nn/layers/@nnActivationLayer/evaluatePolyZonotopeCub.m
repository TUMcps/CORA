function [c, G, Grest, expMat, id, id_, ind, ind_] = evaluatePolyZonotopeCub(obj, c, G, Grest, expMat, id, id_, ind, ind_, evParams)
% evaluatePolyZonotopeCub - evaluates the activation layer on a 
%    polyZonotope using polynomials of cubic order
%
% Syntax:
...
G_ = zeros(num_neurons, nchoosek(3+h, h)-1);
...
for j = 1:num_neurons
    [c_(j), G_(j, :), Grest_(j, :), d(j)] = ...
        obj.evaluatePolyZonotopeNeuronCub(c(j), G(j, :), Grest(j, :), expMat, ...
...
expMat = [expMat, nnHelper.squareExpMat(expMat), nnHelper.cubeExpMat(expMat)];
