...
%------------- BEGIN CODE --------------
    ...
    for i = 1:nrOps
        ...
        if length(list) == 1 || (rand() > 0.5 && cntUn < nrOps - nrBin)  
            ...
            if isempty(nrNest) || nestedOps(list{ind}) < nrNest
            ...
            switch op
                case '~'
                    list{ind} = ~list{ind};
                case 'next'
                    time = min(timeNext,randomTime(tFinal,dt));
                    timeNext = max(0,timeNext - time);
                    list{ind} = next(list{ind},time);
                case 'globally'
                    time = randomTimeInterval(tFinal,dt);
        ...
        % binary operation
            ...
            if isempty(nrNest) || ...
                   nestedOps(list{ind1}) + nestedOps(list{ind2}) < nrNest
...
function res = randomTime(tFinal,dt)
...
function res = randomTimeInterval(tFinal,dt)
% generate random time interval

    time1 = randomTime(tFinal,dt);
    time2 = randomTime(tFinal,dt);

    res = interval(min([time1,time2]),max([time1,time2]));
end

function res = nestedOps(obj)
% determine the number of nested temporal operators
    ...
    elseif strcmp(obj.type,'finally') || ...
        ...
        res = nestedOps(obj.lhs) + 1;

    elseif strcmp(obj.type,'until') || strcmp(obj.type,'release')

        res = nestedOps(obj.lhs) + nestedOps(obj.rhs) + 1;

    elseif strcmp(obj.type,'~')

        res = nestedOps(obj.lhs);

    elseif strcmp(obj.type,'&') || strcmp(obj.type,'|')

        res = nestedOps(obj.lhs) + nestedOps(obj.rhs);
