function [res,list] = stl2rtl(obj,dt)
...
%------------- BEGIN CODE --------------
    ...
    for i = 1:length(clauses)
        ...
        [rtlTmp,listTmp] = convertClause(clauses{i});
...
function [res,listRes] = convertClause(obj)
% convert a single clause to RTL according to Lemma 2 in [1]
    ...
    for i = 1:length(clauses)
        if strcmp(clauses{i}.type,'next')
            if length(list) <= clauses{i}.from
                list{clauses{i}.from+1} = {clauses{i}.lhs};
            else
                list{clauses{i}.from+1} = ...
                                  [list{clauses{i}.from};{clauses{i}.lhs}];
            end
        else
            if length(list) < 1
                list{1} = clauses(i);
