...
%------------- BEGIN CODE --------------
    ...
    for r = 1:l 
        if r == 1 || infi(r) ~= infi(1) || sup(r) ~= sup(1) 
            ...
            Vx = InverseVx(n,dom(r));

            % Compute inverse of W_x
            Wx = InverseWx(n,dom(r),C);
    ...
    for j = 1:h
        for r = 1:l       
            A{j} = transposeMatrix(A{j},r,len,l,n);
            A{j} = M{r}*A{j};
            A{j} = transposeMatrix(A{j},r,len,l,n);
        end
    end
    ...
    if nargout > 1
        ...
        expMat = combinator(n+1,l)' - 1;
        B = zeros(h,size(expMat,2));
        counter = 1;
...
function C = Binomial_coefficient(n)
...
function ind = getIndices(e,len,n)
...
function A = transposeMatrix(A,r,len,l,n)
...
% equation (63) in [1]
    ...
    elseif r ~= 1
        if r == l
            ...
            temp = cellfun(@transpose_,temp,'UniformOutput',false);
            A = [temp{:}];
        end
    end
end

function A = transpose_(A)
