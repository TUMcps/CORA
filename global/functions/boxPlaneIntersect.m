...
%   check for translation (lower bound must be zero at all entries)
    if ~isequal(B(:,1),zeros(dimensionality,1))
        do_translation = 1;
        offset = B(:,1)';
        [B,alpha] = boxTranslation(B,alpha);
    end
    
%   2. get vertices (dim = dim of hyperbox)
%   reminder: now ordered hyperbox located at origin
%   hyperplane given as sum(i,n) x_i = alpha
    vertices = exploreBN(B, alpha);

%   3. scale/translate back if needed
%   first translation
    if do_translation == 1
        vertices = translateSolution(vertices,offset,dimensionality);
    end
%   then scaling
    if do_scaling == 1
        vertices = scaleSolution(vertices,beta,dimensionality);
    end
end

function L = exploreBN(B, alpha)
...
%       additional constraint: if sum of w is bigger than alpha, skip
        if sum(w) <= alpha
            newsol = findSolutions(w, B, alpha);
            if ~isempty(newsol)
...
function intersectionPoints = findSolutions(v, B, alpha)
%% pseudocode from the paper:
...
function scaledBox = boxScaling(B,beta)
%% description:
...
function [translatedBox,alpha_star] = boxTranslation(B,alpha)
%% description:
%   auxiliary function for the scaling of the box
%   this is needed since exploreBN and findSolutions
%   only work with ordered hyperboxes at the origin
...
function scaledSolution = scaleSolution(solution,vector,dimensionality)
%% description:
...
function translatedSolution = translateSolution(solution,vector,dimensionality)
%% description:
...
function stack = fillStack(stack,curr_dim,total_dim,start_index,last_index)
%   recursive function for filling the stack with 0, 1, -1 (for '*')
    if (curr_dim <= total_dim)
        ...
        % recursive call
        stack = fillStack(stack,curr_dim+1,total_dim,new_start,new_last);
    end
end

function partition = createPartition(v, v_star)
%% description:
...
%   v_star needed for copying (to be deleted later)
    v_star_set(1,:) = v_star;
    v_star_set = fillStack(v_star_set,numdef_v_star+1,dimension,2,1);
...
%   get all from i=j+1 to k (between v and v')
    ...
    for i=numdef_v+1:numdef_v_star
        if v_star(i) == 1
            ...
            v_set(1,:) = new_v;
            v_set = fillStack(v_set,i+1,dimension,2,1);
