...
%------------- BEGIN CODE --------------
    ...
    elseif n == length(X)               % use method from [1]
        ...
        Inner = @(f,df,X,X_,Y_) InnerGeneral(f,df,X,X_,Y_);
    end
...
function res = InnerSpecial(f,df,X,X_,Y_)
...
function res = InnerGeneral(f,df,X,X_,Y_,varargin)
...
% the function
    ...
    if any(any(C - eye(n)))
        res = InnerGeneral(f,df,X,X_,Y_,'none');
    else
        res = false;
    end
end

function D = Gamma(A,u,b)
% implementation of the function /Gamma() acc. to Corollary 3.1 in [1]
    D = InvDiag(A)*(b - OffDiag(A)*u);
end
