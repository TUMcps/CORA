...
paramsList(end+1,1) = add2list('tFinal','mandatory',{@isscalar,@(val)ge(val,params.tStart)},...
    {'isscalar','getStart'});
paramsList(end+1,1) = add2list('R0','mandatory',{@(val)any(ismember(getMembers('R0'),class(val))),...
...
paramsList(end+1,1) = add2list('u','default',{@isnumeric,@(val)eq(size(val,1),sys.nrOfInputs),...
