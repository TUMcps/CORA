...
optionsList(end+1,1) = add2list('timeStepInner','optional',{@isscalar,@isnumeric,@(val)abs(rem(params.tFinal,val))<eps},...
    {'isscalar','isnumeric','intsteps'});
optionsList(end+1,1) = add2list('contractor','default',{@ischar,@(val)any(ismember(getMembers('contractor'),val))},...
    {'ischar','membercontractor'});
optionsList(end+1,1) = add2list('iter','default',{@isscalar,@isnumeric,@(val)mod(val,1)==0,@(val)ge(val,1)},...
    {'isscalar','isnumeric','integer','geone'});
optionsList(end+1,1) = add2list('splits','default',{@isscalar,@isnumeric,@(val)mod(val,1)==0,@(val)ge(val,1)},...
    {'isscalar','isnumeric','integer','geone'});
optionsList(end+1,1) = add2list('scaleFac','default',{@(val)c_scaleFac(val,sys,options)},{''});
optionsList(end+1,1) = add2list('orderInner','default',{@isscalar,@isnumeric,@(val)ge(val,1)},{'isscalar','isnumeric','geone'});
optionsList(end+1,1) = add2list('inpChanges','default',{@isscalar,@isnumeric,@(val)mod(val,1)==0,@(val)ge(val,0),...
