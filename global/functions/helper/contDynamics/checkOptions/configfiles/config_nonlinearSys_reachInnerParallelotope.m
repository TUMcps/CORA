...
optionsList(end+1,1) = add2list('alg','mandatory',{@ischar,@(val)any(ismember(getMembers('alg'),val))},{'ischar','memberalg'});
optionsList(end+1,1) = add2list('tensorOrder','mandatory',{@isscalar,@(val)mod(val,1)==0,@(val)c_tensorOrder(val,sys,options)},...
    {'isscalar','integer',''});
