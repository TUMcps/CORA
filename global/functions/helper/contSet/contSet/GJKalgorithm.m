...
% Example:
%    
%    S1 = mptPolytope([0.5 2 4; 1 3 1]');
%    S2 = mptPolytope([2.5 3 6 6; 2 3.5 3.5 1]');
% 
%    GJKalgorithm(S1,S2)
%
...
%------------- BEGIN CODE -------------
    ...
    while true
        ...
        [simplex,d,containsOrigin] = nearestSimplex(simplex);
...
function [simplex,d,containsOrigin] = nearestSimplex(simplex)
...
% simplex that is closest to the origin
    ...
    if size(simplex,2) > size(simplex,1)
        ...
        if n > 3
            index = minDistOriginQuadProg(poly);
        else
            index = minDistOriginRecursive(poly);
        end
        
        % construct next simplex and next search direction
        d = poly.P.A(index,:)';
        
        [~,ind] = sort(abs(poly.P.A(index,:)*simplex - poly.P.b(index)));
...
function index = minDistOriginQuadProg(poly)
% compute the minimum distance between a simplex and the origin using
% quadratic programming

    % object properties
    A = poly.P.A; b = poly.P.b; ind = 1:size(A,1); n = size(A,2);
...
function [index,point] = minDistOriginRecursive(poly)
...
% recusively decomposing the simplex into lower dimensional simplices
    ...
    % initialization
    dist = inf;
    A = poly.P.A; b = poly.P.b; V = vertices(poly); n = size(A,2);
    ...
    for i = 1:size(A,1)
        ...
        if b(i) <= 0
            ...
            if ~contains(poly_,p(2:end))
                [~,p_] = minDistOriginRecursive(poly_ - p(2:end)); 
