...
layers = num2cell(nn_dlt.Layers);
nn_cora = neuralNetwork.convertDLToolboxNetwork(layers);
...
assert(compareMatrices(pred_dlt, pred_cora, 1e-5));
...
% propagate set
options = struct();
options.nn.maxpool_type = "project"; 
pred = nn_cora.evaluate(pZ, options);
