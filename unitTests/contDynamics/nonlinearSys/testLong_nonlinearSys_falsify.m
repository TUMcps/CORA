function res = testLong_nonlinearSys_falsify
% testLong_nonlinearSys_falsify - unit test for falsification
%
% Syntax:
%    res = testLong_nonlinearSys_falsify
%
% Inputs:
%    -
%
...
%     R = reach(sys,params,options);
    ...
    for i = 1:length(alg)
        ...
        I = interval([2.2;2;-10*ones(4,1)],[3.2;2.8;10*ones(4,1)]);
        ...
        x = stl('x',6);
        phi = not(finally(globally(x(1) < 1.5,interval(0,100)),interval(0,300)));
        
        [res,fals] = falsify(sys,params,options,phi);
    
        assert(res == false);
        ind = find(fals.traj.x(1,:) < 1.5);
        assert(max(fals.traj.t(ind)) - min(fals.traj.t(ind)) < 100);
    
        % Test 8: temporal logic (falsifiable)
        x = stl('x',6);
        phi = not(finally(globally(x(1) < 1.6,interval(0,100)),interval(0,300)));
