function res = testLong_nonlinearSys_falsifyRandom
% testLong_nonlinearSys_falsifyRandom - unit test for falsification 
%    featuring random hybrid automata
%
% Syntax:
%    res = testLong_nonlinearSys_falsifyRandom
%
% Inputs:
%    -
%
    ...
    for n = [1 2 3]
        for m = [0 1 2]
            for p = [1 2]
                ...
                % create random nonlinear system
                B = []; D = [];
                if m > 0
                    B = rand(n,m); D = rand(p,m);
                end

                linsys = linearSys(rand(n),B,rand(n,1),rand(p,n),D,rand(p,1));
                sys = nonlinearSys(linsys);
