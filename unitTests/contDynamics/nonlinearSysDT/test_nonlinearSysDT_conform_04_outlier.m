function res = test_nonlinearSysDT_conform_04_outlier
% test_nonlinearSysDT_conform_04_outlier - simple unit test for outlier
%   detection in reachset-conformant identification
%
% Syntax:
%    test_nonlinearSysDT_conform_04_outlier
%
% Inputs:
%    -
%
...
for n_out = [1 3]
    ...
    % test outlier detection methods "RMSE", "MILP", "search", "searchG"
    ...
    assert(results_Rgen.fval <= fval0 + tol) % cost should not increase
    ...
    assert(fval_opt <= results_Rgen.fval + tol) % cost should be optimal
    assert(numCont >= (n_m*n_s - n_out)) % not more than n_out outliers

    % search: iterative search for optimal outlier removal
    options.cs.outMethod = "search";
    options.cs.constraints = "gen";
    [params_Sgen, results_Sgen] = conform(sys,params_id_init,options);
    numCont = aux_evaluateCoverage(sys,params_Sgen,n_k);
    assert(withinTol(results_Sgen.fval,fval_opt, tol)) % cost should be optimal
    assert(numCont >= (n_m*n_s - n_out)) % not more than n_out outliers

    % use halfspace constraints
    options.cs.constraints = "half";
    [params_Shalf, results_Shalf] = conform(sys,params_id_init,options);
    numCont_Shalf = aux_evaluateCoverage(sys,params_Shalf,n_k);
    assert(withinTol(results_Shalf.fval,fval_opt, tol)) % cost should be optimal
    assert(numCont_Shalf >= (n_m*n_s - n_out)) % not more than n_out outliers

    % searchG: greedy search for outlier removal
    options.cs.outMethod = "searchG";
    options.cs.constraints = "gen";
    [params_SGgen, results_SGgen] = conform(sys,params_id_init,options);
    numCont = aux_evaluateCoverage(sys,params_SGgen,n_k);
    if n_out == 1
        assert(withinTol(results_SGgen.fval,fval_opt, tol)) % cost should be optimal
    else
        assert(results_SGgen.fval >= fval_opt - tol)
        assert(results_SGgen.fval <= fval0 + tol) % cost should not increase
    end
    assert(numCont >= (n_m*n_s - n_out)) % not more than n_out outliers

    options.cs.constraints = "half";
    [params_SGhalf, results_SGhalf] = conform(sys,params_id_init,options);
    numCont = aux_evaluateCoverage(sys,params_SGhalf,n_k);
    if n_out == 1
        assert(withinTol(results_SGhalf.fval,fval_opt, tol)) % cost should be optimal
    else
        assert(results_SGhalf.fval >= fval_opt - tol)
        assert(results_SGhalf.fval <= fval0 + tol) % cost should not increase
    end
    assert(numCont >= (n_m*n_s - n_out)) % not more than n_out outliers
end
end
