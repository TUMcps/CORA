function res = testLong_nonlinearSysDT_identify
% testLong_nonlinearSysDT_identify - unit test for system identification
%
% Syntax:
%    res = testLong_nonlinearSysDT_identify
%
% Inputs:
%    -
%
    ...
    for i = 1:length(alg)
        %for j = 1:length(basis)
            for k = 1:2
                ...
                try
                sys = nonlinearSysDT.identify(x1,t1,options);
                catch
                end
                assert(isa(sys,'nonlinearSysDT'));
    
                % Test 2: single trajectory with input
                sys = nonlinearSysDT.identify(x1,t1,u1,options);
                assert(isa(sys,'nonlinearSysDT'));
