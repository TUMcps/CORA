...
[V_c,~,~,~] = priv_subspace_Krylov_Saad(linDyn,options.R0,options);
% obtain Krylov order
krylovOrder = length(V_c(1,:));
...
for i=1:length(t)
    ...
    error = aux_KrylovError(linDyn.A,options.x0,t(i),krylovOrder,options);
...
function error = aux_KrylovError(A,v,t_f,krylovOrder,options)
