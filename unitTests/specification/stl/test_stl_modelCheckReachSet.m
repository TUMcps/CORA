...
% Last update:   16-February-2024 (FL, add incremental algorithm)
% Last revision: ---
...
% model checking
for i = 1:length(alg)
    assertLoop(~modelChecking(R,eq,alg{i}),i)
end
...
dyn = linearSys([0 0; 0 0], [0;0], [1;1]);

reset = linearReset.eye(2);

guard = polytope([],[],[1 0],5);
...
% model checking
for i = 1:length(alg)
    if ismember(alg{i}, {'rtl', 'sampledTime'})
