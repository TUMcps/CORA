...
try
    ...
    resvec(end+1) = dim(X0{1}) == 28*28 && all(size(spec.set.P.A) == [9,10]);
    
    % axas_xu benchmark
    [X0, spec] = vnnlib2cora('axas_xu_prop_3.vnnlib');
    resvec(end+1) = dim(X0{1}) == 5 && all(size(spec.set.P.A) == [4,5]);
    
    % rl_learning benchmark
    [X0, spec] = vnnlib2cora('rl_benchmark_dubinsrejoin_case_unsafe_11.vnnlib');
    resvec(end+1) = dim(X0{1}) == 8 && all(size(spec.set.P.A) == [6,8]);
