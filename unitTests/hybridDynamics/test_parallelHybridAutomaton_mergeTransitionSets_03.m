...
guard = conHyperplane([1 1 1],1);
reset = struct('A',[1 0 0; 0 1 0; 0 0 1],'c',[-1;-1;-1]);
trans{2,2}{1} = transition(guard,reset,1);
...
trans{3,1}{1} = transition(guard,reset,2,'sync2');
...
% guard set
guard = projectHighDim(mptPolytope(trans{1,locID(1)}{1}.guard),7,1:2) ...
    & projectHighDim(mptPolytope(trans{2,locID(2)}{1}.guard),7,3:5);

% reset function: identity for third component
reset.A = blkdiag(trans{1,locID(1)}{1}.reset.A,...
    trans{2,locID(2)}{1}.reset.A,...
    eye(length(trans{3,locID(3)}{1}.reset.A)));
reset.c = [trans{1,locID(1)}{1}.reset.c;...
    trans{2,locID(2)}{1}.reset.c;...
    zeros(length(trans{3,locID(3)}{1}.reset.c),1)];
...
% guard set
guard = projectHighDim(trans{3,locID(3)}{1}.guard,7,6:7);

% reset function: identity for first/second component
reset.A = blkdiag(eye(length(trans{1,locID(1)}{1}.reset.A)),...
    eye(length(trans{2,locID(2)}{1}.reset.A)),...
    trans{3,locID(3)}{1}.reset.A);
reset.c = [zeros(length(trans{1,locID(1)}{1}.reset.c),1);...
    zeros(length(trans{2,locID(2)}{1}.reset.c),1);...
    trans{3,locID(3)}{1}.reset.c];
...
% compare solutions
if length(mergedTrans) ~= length(mergedTrans_)
    res = false;
end
if ~isequal(mergedTrans{1},mergedTrans_{1},1e-14)
    res = false;
end
if ~isequal(mergedTrans{2},mergedTrans_{2},1e-14)
    res = false;
end
