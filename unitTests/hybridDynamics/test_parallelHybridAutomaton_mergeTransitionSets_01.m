...
trans{1,1}{1} = transition(guard,reset,2);
...
guard = conHyperplane([1 1],0);
reset = struct('A',[1 0; 0 1],'c',[-3;3]);
trans{1,2}{1} = transition(guard,reset,1);
...
trans{2,1}{1} = transition(guard,reset,2);
...
for i=1:size(comb,1)
    ...
    % guard set
    guard = conHyperplane([trans{1,comb(i,1)}{1}.guard.h.c;...
        zeros(length(trans{2,comb(i,2)}{1}.guard.h.c),1)],0);

    % reset function
    reset = struct('A',eye(5),'c',...
        [trans{1,comb(i,1)}{1}.reset.c;...
        zeros(length(trans{2,comb(i,2)}{1}.reset.c),1)]);
    ...
    % guard set
    guard = conHyperplane([zeros(length(trans{1,comb(i,1)}{1}.guard.h.c),1);...
        trans{2,comb(i,2)}{1}.guard.h.c],1);

    % reset function
    reset = struct('A',eye(5),'c',...
        [zeros(length(trans{1,comb(i,1)}{1}.reset.c),1);...
        trans{2,comb(i,2)}{1}.reset.c]);
