...
% HA1: transitions
trans1{1} = transition(guard_right,reset_up,2,'hit_right');
trans2{1} = transition(guard_left,reset_up,1,'hit_left');

% HA1: locations
loc{1} = location('right',inv,trans1,linSys_right);
loc{2} = location('left',inv,trans2,linSys_left);

% instantiate first hybrid automaton
HA1 = hybridAutomaton(loc);

% HA2: transitions
trans1{1} = transition(guard_empty,reset_left,2,'hit_right');
trans2{1} = transition(guard_empty,reset_left,1,'hit_left');

% HA2: locations (invariants are full space)
loc{1} = location('up',inv_empty,trans1,linSys_up);
loc{2} = location('down',inv_empty,trans2,linSys_down);
