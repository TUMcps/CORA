function res = testLongDuration_zonotope_intersectStrip
% testLongDuration_zonotope_intersectStrip - unit test function of 
% intersectStrip
%
% Syntax:  
%    res = testLongDuration_zonotope_intersectStrip
%
% Inputs:
%    -
%
...
for i = 1:length(Z_over)
    resVec(i) = in(mptPolytope(Z_over{i} + Z_tol),P_exact);
end
% case for single strip
for i = 1:length(Z_over_single)
    resVec(i+length(Z_over)) = in(mptPolytope(Z_over_single{i} + Z_tol),P_exact_single);
end
