function res = test_conZonotope_quadMap
% test_conZonotope_quadMap - unit test function for the
%                                            computation of the quadratic
%                                            map of a constrained zonotope
%
% Syntax:  
%    res = test_conZonotope_quadMap
%
% Inputs:
%    -
%
...
%     plot(cZquad,[1,2],'r');
    ...
    for i = 1:size(points_,2)
       if any(A*points_(:,i) - b > Tol)
           file_name = strcat('test_conZonotope_quadMap_1_', ...
...
for k = 2:5
    ...
    if max(max(abs(zonoQuad.Z - cZquad.Z))) > 1e-18 || ...
       ~isempty(cZquad.A) || ~isempty(cZquad.b)
        file_name = strcat('test_conZonotope_quadMap_2_', ...
                           datestr(now,'mm-dd-yyyy_HH-MM'));
...
%     plot(cZquad,[1,2],'r');
    ...
    for i = 1:size(points_,2)
       if any(A*points_(:,i) - b > Tol)
           file_name = strcat('test_conZonotope_quadMap_3_', ...
                              datestr(now,'mm-dd-yyyy_HH-MM'));
