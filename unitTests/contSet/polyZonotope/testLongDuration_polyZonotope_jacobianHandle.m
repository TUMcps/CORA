function res = testLongDuration_polyZonotope_jacobianHandle
% testLongDuration_polyZonotope_jacobianHandle - unit test function for
%    computing the fhandle of the jacobian of a polyZonotope
%
% Syntax:  
%    res = testLongDuration_polyZonotope_jacobianHandle
%
% Inputs:
%    -
%
...
for i=1:nTests
    ...
    f_jac = jacobianHandle(pZ,pZ.id(ind_diff));
...
if ~res
    disp('testLongDuration_polyZonotope_jacobianHandle failed');
else
    disp('testLongDuration_polyZonotope_jacobianHandle successful');
end
%------------- END OF CODE --------------