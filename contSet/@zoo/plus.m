...
%------------- BEGIN CODE --------------
    ...
    if isa(factor1, 'zoo') && isa(factor2, 'zoo')
        ...
        res = arrayfun(@(a, b) s_plus_zz(a, b), factor1, factor2, 'UniformOutput', false);

    elseif isa(factor1,'zoo') && isa(factor2,'double')

        res = arrayfun(@(a) s_plus_zd(a, factor2), factor1, 'UniformOutput', false);
    
    elseif isa(factor1,'double') && isa(factor2,'zoo')
        
        res = arrayfun(@(b) s_plus_dz(factor1, b), factor2, 'UniformOutput', false);
