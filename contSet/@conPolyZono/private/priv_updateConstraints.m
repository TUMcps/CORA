function cPZ = priv_updateConstraints(cPZ,cPZ1,cPZ2)
% priv_updateConstraints - Update constraints after combining two 
...
% Last update:   16-October-2024 (TL, fix ids)
% Last revision: ---
...
if isempty(cPZ1.A)
    if ~isempty(cPZ2.A)
        zeroPad1 = zeros(length(cPZ1.id),size(cPZ2.EC,2));
        cPZ.EC = [zeroPad1;cPZ2.EC];
    end
else
    if isempty(cPZ2.A)
        zeroPad2 = zeros(length(cPZ2.id),size(cPZ1.EC,2));
        cPZ.EC = [cPZ1.EC;zeroPad2];
    else
