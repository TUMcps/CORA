function E = priv_orEllipsoidOA(E_cell)
...
max_val = max(cellfun(@(E_i) max(svd(E_i.Q)),E_cell,'UniformOutput',true));
...
% if any ellipsoid is degenerate, add small pertubation (possible since we
...
for i=1:N
    ...
    if ~isFullDim(E_i)
        ...
        E_cell{i} = ellipsoid(Ti*Si*Ti',E_i.q);
    end
end
...
elseif isSDPT3
    ...
    for i=1:N
        ...
        % li
        Qi_inv = inv(E_cell{i}.Q);
        ci = E_cell{i}.q'*Qi_inv*E_cell{i}.q-1;
        ...
        a_val_li = [-Qi_inv_vec,sqrt(2)*(Qi_inv*E_cell{i}.q)',-ci];
