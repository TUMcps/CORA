function E = priv_plusEllipsoidOA(E_cell)
% priv_plusEllipsoidOA - computes the smallest-volume outer-approximation
%    of the Minkowski sum of ellipsoids
%
% Syntax:
%    E = priv_plusEllipsoidOA(E_cell)
%
% Inputs:
...
%
% References:
%   [1] S. Boyd et al. "Linear matrix inequalities in system and control
%       theory"
%
...
for i=1:N
    if ~isFullDim(E_cell{i})
        % bloat ellipsoid in appropriate dimensions
        [U,S,~] = svd(E_cell{i}.Q);
        nt = rank(E_cell{i});
        s = diag(S);
        s(nt+1:end) = 2*max(s(1)*E_cell{i}.TOL,E_cell{i}.TOL);
        Q_i = U*diag(s)*U';
        E_cell{i}.Q = 1/2*(Q_i+Q_i)';
    end
end
...
for i=1:N
    ...
    E0 = E0 + E_c{i};
    b_i = -E_cell{i}.Q\E_cell{i}.q;
    At_c{i} = E_c{i}'*(E_cell{i}.Q\E_c{i});
    bt_c{i} = E_c{i}'*b_i;
    c_c{i} = E_cell{i}.q'*(E_cell{i}.Q\E_cell{i}.q)-1;
end
...
if isMosek
    ...
    % For details, see "orEllipsoidOA.m" (very similar)
