function [pZsplit] = splitOneGen(pZ,genNr,varargin)
% splitOneGen - Splits one generator factor of a polynomial zonotope
%
% Syntax:  
%    [pZsplit] = splitOneGen(pZ,genNr)
%    [pZsplit] = splitOneGen(pZ,genNr,polyOrd)
%
% Inputs:
%    pZ - polyZonotope object
%    genNr - generator number for the splitting (has to be a generator of
...
% check if the selected generator has a polynomial order of one
exp = pZ.expMat(:,genNr);
if sum(exp) == 1
   index = find(exp == 1);
else
