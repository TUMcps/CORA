function res = contains(pZ,S,varargin)
% contains - determines if a polynomial zonotope contains a set or a point
%
% Syntax:  
%    res = contains(pZ,S)
%    res = contains(pZ,S,type)
%
% Inputs:
...
%------------- BEGIN CODE --------------
    ...
    % pre-processing
    [res,vars] = pre_contains('polyZonotope',pZ,S,varargin{:});
    ...
    if isnumeric(S)
        ...
        for i=1:size(S,2)
            ...
            res(i) = containsInterval(fHan,jacHan,X,X_,Y);
        end
    ...
    elseif isa(S,'interval') || isa(S,'zonotope') || ...
        ...
        if disproveContainment(pZ,S)
            return; 
        end
        
        % try to prove that set containment holds
        res = proveContainment(S,fHan,jacHan,X);
