function [pChange,plotInfo]=priv_laneChangeProbabilityOptimized(simOptions,selectionVector,pLeft)
% priv_laneChangeProbabilityOptimized - simulates a vehicle changing lanes on a multi-lane road
%
% Syntax:
%    [p,pTotal]=priv_laneChangeProbabilityOptimized(simOptions,markovChainSpec)
%
% Inputs:
...
motLeft=priv_motivationOptimized(wFrontProb,selectionVector.left{iStep},pLeft.T{iStep},simOptions);

%compute motivation value for changing to the right lane
[motRight]=priv_motivationOptimized(wFrontProb,selectionVector.right{iStep},pLeft.T{iStep},simOptions);
...
motBehind=priv_motivationOptimized(wBehindProb,selectionVecBehind,pBehind.T{iStep},simOptions);
