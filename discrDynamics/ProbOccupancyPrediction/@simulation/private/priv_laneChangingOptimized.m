function [p,pTotal,lcEvolProb]=priv_laneChangingOptimized(simOptions,markovChainSpec)
% priv_laneChangingOptimized - simulates a vehicle changing lanes on a multi-lane road
%
% Syntax:
%    [p,pTotal]=priv_laneChangingOptimized(simOptions,markovChainSpec)
%
% Inputs:
...
selectionVec.left=aux_selectionVector(runs,ThetaC,pFrontLeft);
selectionVec.right=aux_selectionVector(runs,ThetaC,pFrontRight);
...
pLeft.T{1}=aux_re_shape(pLeft.T{1});
pRight.T{1}=aux_re_shape(pRight.T{1});


for iStep=1:runs
   %compute lane changing probability
   [pChange,plotInfo{iStep}]=priv_laneChangeProbabilityOptimized(simOptions,selectionVec,pLeft);
   ...
   %compute pLeft
   [pLeftNew,pTotalLeftNew]=priv_singleStepDrivingOptimized(simOptions,pLeft);
   ...
   pTmp2=aux_re_shape(pTmp2);
   
   %add changing probability
   pRight.T{iStep}=pRight.T{iStep}+pChange*pTmp2;   
   %compute pRight
   [pRightNew,pTotalRightNew]=priv_singleStepDrivingOptimized(simOptions,pRight);
...
%    ratio.right(iStep)=sum(sum(pChange*pLeft.T{iStep}))/sum(sum(pRight.T{iStep}));
   ...
   lcEvolProb=priv_laneChangeEvolution(pChange,lcEvolProb,markovChainSpec);
end
...
function [m]=aux_selectionVector(runs,ThetaC,pFront)
