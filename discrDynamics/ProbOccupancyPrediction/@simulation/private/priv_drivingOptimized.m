function [p,pTotal,pTrans]=priv_drivingOptimized(simOptions,markovChainSpec)
% priv_drivingOptimized - simulates a vehicle where the selection matrix has been
...
for iStep=1:(simOptions.runs) 
    ...
    if ~isempty(nonZeroIndTmp)
        ...
        mPartial=priv_combineOptimized(mPartial,freeDriving(nonZeroIndTotal),nrOfInputs); 
