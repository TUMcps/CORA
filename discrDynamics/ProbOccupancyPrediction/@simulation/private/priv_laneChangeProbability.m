function [pChange,plotInfo]=priv_laneChangeProbability(simOptions,markovChainSpec,selectionVector,pLeft)
% priv_laneChangeProbability - simulates a vehicle changing lanes on a multi-lane road
%
% Syntax:
%    [p,pTotal]=priv_laneChangeProbability(simOptions,markovChainSpec)
%
% Inputs:
...
motLeft=priv_motivation(wFrontProb,selectionVector.left{iStep},pLeft.T{iStep},simOptions);

%compute motivation value for changing to the right lane
[motRight]=priv_motivation(wFrontProb,selectionVector.right{iStep},pLeft.T{iStep},simOptions);

%NEW FOLLOWING VEHICLE
pLeftAct.T{1}=pLeft.T{iStep};
selectionVecBehind=aux_singleStepSelectionVector(simOptions,nrOfInputs,z,Omega,pLeft.T{end});
%compute motivation value for vehicle driving behind
motBehind=priv_motivation(wBehindProb,selectionVecBehind,pBehind.T{iStep},simOptions);
...
function [m]=aux_singleStepSelectionVector(simOptions,nrOfInputs,z,Omega,pFront)
